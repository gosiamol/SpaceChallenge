...
public class Game {
    ...
    public String checkGameWinner(char [][]grid){
        String result = "None";
        //jezeli 3x w pionie i 3x w poziomie i 2 na krzyz
        int i,j,k;
        char player ;
        for ( k=0; k < 2; k++) {
            if (k == 0) player = 'x';
            else if (k == 1) player = 'o';
            else break;
            //w pionie
            for (j = 0; j < 3; j++) {
                for (i = 0; i < 3; i++) {
                   ...
                   if( i == 2 ) result = Character.toUpperCase(player)+" wins";
                }
            }
            //w poziomie
            for (j = 0; j < 3; j++) {
        ...
        if ( k==0 && result.equals("None")) result= "Tie";
